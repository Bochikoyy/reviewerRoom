...
function lightboxView(){
 ...
 return `<div class="lightbox-overlay" data-action="close-lightbox" role="dialog" aria-modal="true" aria-label="Image viewer"><div class="lightbox-content" onclick="event.stopPropagation()"><div class="lightbox-image-area"><img class="lightbox-image" src="${esc(lightboxFig.src)}" alt="${esc(lightboxFig.alt||'')}"></div><aside class="lightbox-notes-panel"><div class="lightbox-notes-header"><h3>${icon('info')} Figure Notes</h3><button class="icon-btn lightbox-close" data-action="close-lightbox" aria-label="Close image viewer">${icon('close')}</button></div><div class="lightbox-notes-body">${lightboxFig.caption?`<div class="lightbox-caption">${lightboxFig.caption}</div>`:''}${lightboxFig.alt?`<div class="lightbox-alt"><span class="eyebrow">DESCRIPTION</span><p>${esc(lightboxFig.alt)}</p></div>`:''}<div class="lightbox-tip">${icon('sun')}<p>Use this view to study cell morphology details. Look for key identifying features described in the caption.</p></div></div></aside></div></div>`;
}
...
app.addEventListener('click',e=>{
 ...
 if(action==='close-lightbox'){lightboxFig=null;document.querySelector('.lightbox-overlay')?.remove();document.body.style.overflow='';return}

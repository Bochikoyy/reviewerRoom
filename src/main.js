...
let search='',bookmarksOnly=false,sidebarOpen=false,cardIndex=0,flipped=false,cardFilter='all',cardOrder=null,reviewOnly=false;
...
function render(){
 ...
 <footer class="content-footer"><span>${icon('sun')} A little progress is still progress.</span><span>Made for curious minds · Sunroom</span></footer></main>`;
...
document.addEventListener('keydown',e=>{
 if(e.key==='Escape'){if(sidebarOpen){sidebarOpen=false;render();document.querySelector('[data-action="menu"]')?.focus()}if(timerOpen){timerOpen=false;renderTimer();document.querySelector('.timer-fab')?.focus()}return}

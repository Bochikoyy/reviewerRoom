import './style.css';
import { lessons, pathologyLessons, industryElectiveLessons, subjects } from './data.js';

if('serviceWorker' in navigator){
 window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js').catch(()=>{}));
}
window.addEventListener('online',()=>{updateConnectionStatus();toast('You’re back online.');});
window.addEventListener('offline',()=>{updateConnectionStatus();toast('You’re offline. Your saved study content is still available.');});
...
function parseHash(){
 const m=location.hash.match(/^#(?:(endocrinology|pathology|industry-elective)\/)?lesson-(\d+)\/(notes|flashcards|quiz)$/);
...
let subjectFolders=read('sunroom-subject-folders',{endocrinology:true,pathology:true,industryElective:true});
let endocrinologyOpen=subjectFolders.endocrinology??true;
let pathologyOpen=subjectFolders.pathology??true;
let industryElectiveOpen=subjectFolders.industryElective??true;
...
function source(page,label='View source'){
 const pdfPath=currentSubject==='pathology'?`/lessons/Pathology-${unit}.pdf`:currentSubject==='industry-elective'?`/lessons/Industry-Elective-${unit}.pdf`:`/lessons/Unit-${unit}.pdf`;
...
function sidebar(){
 ...
 return `<div class="sidebar-backdrop ${isDialog?'show':''}" data-action="close-menu"></div><aside class="sidebar ${sidebarOpen?'open':''}" id="subject-sidebar" aria-label="Subject navigation" ${isDialog?'role="dialog" aria-modal="true"':''}>
  ...
  <div class="subject-label">Subjects <span>03</span></div>
   ...
   <section class="subject-folder"><button class="subject-folder-toggle ${currentSubject==='industry-elective'?'selected-subject subject-active':''}" data-action="toggle-subject" data-id="industry-elective" aria-expanded="${industryElectiveOpen}" aria-controls="subject-collection-2" aria-label="${industryElectiveOpen?'Collapse Industry Elective':'Expand Industry Elective'}" title="${industryElectiveOpen?'Collapse Industry Elective':'Expand Industry Elective'}">${icon('code')}<span>Industry Elective</span>${icon('chevron')}</button><div id="subject-collection-2" class="subject-collection" ${industryElectiveOpen?'':'hidden'}><div class="lesson-nav-label eyebrow">THE COLLECTION</div><nav class="lesson-nav" aria-label="Industry Elective lessons">${(subjects['industry-elective']?.lessons||[]).map(l=>`<button data-action="lesson" data-subject="industry-elective" data-id="${l.id}" class="lesson-link ${currentSubject==='industry-elective'&&unit===l.id?'selected':''}" ${currentSubject==='industry-elective'&&unit===l.id?'aria-current="page"':''}><span class="lesson-num">0${l.id}</span><span><strong>Lesson ${l.id}</strong><small>${l.sourceTitle}</small></span>${progress.read[`industry-elective-${l.id}`]?icon('check','read-check'):''}</button>`).join('')}</nav></div></section>
...
function lightboxView(){
 ...
 return `<div class="lightbox-overlay" data-action="close-lightbox" role="dialog" aria-modal="true" aria-label="Image viewer"><div class="lightbox-content"><div class="lightbox-image-area"><img class="lightbox-image" src="${esc(lightboxFig.src)}" alt="${esc(lightboxFig.alt||'')}"></div><aside class="lightbox-notes-panel"><div class="lightbox-notes-header"><h3>${icon('info')} Figure Notes</h3><button class="icon-btn lightbox-close" data-action="close-lightbox" aria-label="Close image viewer">${icon('close')}</button></div><div class="lightbox-notes-body">${lightboxFig.caption?`<div class="lightbox-caption">${lightboxFig.caption}</div>`:''}${lightboxFig.alt?`<div class="lightbox-alt"><span class="eyebrow">DESCRIPTION</span><p>${esc(lightboxFig.alt)}</p></div>`:''}<div class="lightbox-tip">${icon('sun')}<p>${currentSubject==='pathology'?'Use this view to study cell morphology details. Look for key identifying features described in the caption.':'Use this view to examine diagram details, architecture, and visual structures in depth.'}</p></div></div></aside></div></div>`;
}
function notesView(){return `<div class="notes-layout"><div class="notes-main"><div class="notes-controls"><label class="search-box">${icon('search')}<input id="notes-search" type="search" placeholder="Find a concept in this lesson…" aria-label="Search lesson notes" value="${esc(search)}"><kbd>/</kbd></label><button class="icon-btn bookmark-filter ${bookmarksOnly?'active':''}" data-action="filter-bookmarks" aria-label="Show bookmarked sections" aria-pressed="${bookmarksOnly}">${icon('bookmark')}</button></div><div id="note-sections">${noteSections()}</div></div><aside class="lesson-aside"><div class="on-page"><div class="eyebrow">IN THIS LESSON</div><nav aria-label="Lesson sections">${lesson().sections.map((s,i)=>`<a href="#section-${i}" data-action="section" data-id="${i}"><span>${String(i+1).padStart(2,'0')}</span>${s.title}</a>`).join('')}</nav></div><div class="recall-callout">${icon('cards')}<h3>Make it stick.</h3><p>Close your notes. Try recalling one idea before moving on.</p><button class="text-btn" data-action="mode" data-mode="flashcards">Try the flashcards ${icon('arrow')}</button></div><a class="pdf-download" href="${currentSubject==='pathology'?`/lessons/Pathology-${unit}.pdf`:currentSubject==='industry-elective'?`/lessons/Industry-Elective-${unit}.pdf`:`/lessons/Unit-${unit}.pdf`}" download>${icon('download')}<span>Original lesson PDF<small>${currentSubject==='pathology'?`Pathology Lecture ${unit}`:currentSubject==='industry-elective'?`Industry Elective ${unit}`:`Unit ${unit}`} · ${lesson().pages} pages</small></span></a><p class="source-disclaimer">Study summaries based on your supplied units. Open the original PDF for full detail. ${currentSubject==='industry-elective'?'Code examples and diagrams are for course revision.':'Clinical examples are for course revision.'}</p></aside></div>`}
...
function cardsView(){const deck=cardDeck();cardIndex=Math.min(cardIndex,Math.max(0,deck.length-1));const c=deck[cardIndex];const known=learnedCount(lesson());return `<section class="practice-area"><div class="practice-heading"><div><div class="eyebrow">A LITTLE ACTIVE RECALL</div><h2>Small cards. Lasting knowledge.</h2><p>Think of your answer, then flip to see how you did.</p></div><div class="learned-pill">${icon(subjects[currentSubject]?.icon||'leaf')} ${known} / ${lesson().cards.length} learned</div></div><div class="flash-toolbar"><div class="pill-group"><button data-action="card-filter" data-filter="all" class="${cardFilter==='all'?'active':''}" aria-pressed="${cardFilter==='all'}">All cards <span>${lesson().cards.length}</span></button><button data-action="card-filter" data-filter="review" class="${cardFilter==='review'?'active':''}" aria-pressed="${cardFilter==='review'}">Still learning <span>${lesson().cards.length-known}</span></button></div><button class="text-btn" data-action="shuffle">${icon('shuffle')} Shuffle</button></div>
 ...
 <div class="card-rating"><p>${flipped?'How did that one feel?':'Reveal the answer to track your learning.'}</p><div><button class="btn secondary" data-action="rate-card" data-rating="review" ${flipped?'':'disabled'}>${icon('reset')} Still learning</button><button class="btn primary" data-action="rate-card" data-rating="learned" ${flipped?'':'disabled'}>${icon('check')} Got it!</button></div></div><div class="flash-source">${currentSubject==='pathology'||currentSubject==='industry-elective'?'Lecture':'Unit'} ${unit}, page ${c.page} · ${source(c.page,'Check the original')}</div>`:`<div class="empty-state complete-state">${icon('sun')}<h2>A little sunshine, well earned.</h2><p>You’ve marked all ${lesson().cards.length} cards as learned. Revisit the full deck whenever you like.</p><button class="btn primary" data-action="card-filter" data-filter="all">Review all cards ${icon('arrow')}</button></div>`}
...
app.addEventListener('click',e=>{
 ...
 if(action==='toggle-subject'){
  ...
  }else if(id==='industry-elective'){
   industryElectiveOpen=!industryElectiveOpen;
   subjectFolders.industryElective=industryElectiveOpen;
  }else{
...
const donationOptions=[
 {
  ...
  initialSubtext:'Accepting donations · Minimum 10 pesos 🥰 salamat ha!',
  recurringSubtext:'You’ve been studying hard for 30 minutes! Keep going and consider sending a little support 🥰'
 },
 {
  id:'gcash',
  label:'GCash',
  pill:'GCash (InstaPay) · Chrisnel Graine Caipang',
  src:'/images/donation-qr-instapay.png',
  alt:'GCash InstaPay QR Code for Chrisnel Graine Caipang',
